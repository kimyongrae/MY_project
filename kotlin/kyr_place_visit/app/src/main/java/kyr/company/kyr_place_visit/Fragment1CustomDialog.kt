...
//        dialog.window?.clearFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);//뒷 배경 하얗게

//        fullscreen bottomsheet 참조하기
/*        dialog.window?.setGravity(Gravity.TOP); // 다이얼로그 바텀으로
        
        val metrics: DisplayMetrics = context.resources.displayMetrics
        val displayPixelWidth = metrics.widthPixels
        val displayPixelHeight = metrics.heightPixels

        AppContants.println("$displayPixelWidth,$displayPixelHeight")

        //displayheight값을 먼저 구한후에 파람 y값 설정을 해줘야됨...
